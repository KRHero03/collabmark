 ...
 */
function resolveComment(comment: CommentData, ytext: Y.Text, ydoc: Y.Doc): ResolvedAnchor | null {
  ...
  if (!comment.anchor_from_relative || !comment.anchor_to_relative) {
    if (
      ...
      comment.anchor_from !== undefined &&
      comment.anchor_to !== null &&
      comment.anchor_to !== undefined
    ) {
      const docText = ytext.toString();
      const slice = docText.slice(comment.anchor_from, comment.anchor_to);
      const status: AnchorStatus =
        comment.quoted_text && slice === comment.quoted_text ? "exact" : comment.quoted_text ? "modified" : "exact";
  ...
  try {
    const relFrom = Y.decodeRelativePosition(base64ToUint8(comment.anchor_from_relative));
    const relTo = Y.decodeRelativePosition(base64ToUint8(comment.anchor_to_relative));

    const absFrom = Y.createAbsolutePositionFromRelativePosition(relFrom, ydoc);
...
export function useCommentAnchors({
  ...
  const [anchors, setAnchors] = useState<Map<string, ResolvedAnchor>>(new Map());
  ...
  const resolveAll = useCallback(() => {
    ...
    for (const comment of comments) {
      ...
      if (resolved.status === "orphaned" && !comment.is_orphaned && !orphanQueueRef.current.has(comment.id)) {
      ...
      if (
        ...
        comment.anchor_from !== null &&
        comment.anchor_from !== undefined &&
        (resolved.from !== comment.anchor_from || resolved.to !== comment.anchor_to)
      ) {
    ...
    if (driftedComments.length > 0) {
      ...
      reanchorTimerRef.current = setTimeout(() => {
        for (const { id, from, to } of driftedComments) {
          commentsApi.reanchor(id, { anchor_from: from, anchor_to: to }).catch(() => {});
        }
      }, 2000);
    }
