...
describe("DiffView", () => {
  it("shows 'No differences' message when text is identical", () => {
    const { getByText } = render(
      <DiffView oldText="Hello world" newText="Hello world" />,
    );
    expect(
      getByText("No differences — version matches current document."),
    ).toBeDefined();
  });

  it("renders added lines with data-testid='diff-added'", () => {
    const { getAllByTestId } = render(
      <DiffView oldText="line1\n" newText="line1\nline2\n" />,
    );
  ...
  it("renders mixed changes (additions + removals + unchanged) correctly", () => {
    ...
    if (unchanged.length > 0) {
      expect(unchanged.some((el) => el.textContent?.includes("common"))).toBe(
        true,
      );
    }
  });
  ...
  it("handles empty strings comparison", () => {
    const { getAllByTestId } = render(
      <DiffView oldText="" newText="new line\n" />,
    );
    const added = getAllByTestId("diff-added");
    expect(added.length).toBeGreaterThanOrEqual(1);
  ...
  it("handles single line diff (addition)", () => {
    const { getAllByTestId } = render(
      <DiffView oldText="" newText="single" />,
    );
    const added = getAllByTestId("diff-added");
    expect(added.length).toBeGreaterThanOrEqual(1);
