...
describe("DocumentInfoModal", () => {
  ...
  it("renders nothing when open is false", () => {
    const { container } = render(<DocumentInfoModal doc={baseDoc} open={false} onClose={vi.fn()} />);
    expect(container.innerHTML).toBe("");
  });

  it("renders all metadata fields when open", () => {
    const { getByText } = render(<DocumentInfoModal doc={baseDoc} open onClose={vi.fn()} />);
  ...
  it("shows 'anyone can edit' access level", () => {
    ...
    const { getByText } = render(<DocumentInfoModal doc={doc} open onClose={vi.fn()} />);
  ...
  it("does not show deleted row for non-deleted docs", () => {
    const { queryByText } = render(<DocumentInfoModal doc={baseDoc} open onClose={vi.fn()} />);
  ...
  it("calls onClose when clicking the close button", () => {
    const onClose = vi.fn();
    const { getByText } = render(<DocumentInfoModal doc={baseDoc} open onClose={onClose} />);
  ...
  it("calls onClose when clicking the backdrop", () => {
    const onClose = vi.fn();
    const { container } = render(<DocumentInfoModal doc={baseDoc} open onClose={onClose} />);
